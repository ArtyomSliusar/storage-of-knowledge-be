...
# from main.models import Note, Comment, Subject
# from main.forms import ContactForm, UserForm, SearchNotesForm, NoteForm, EditUserForm, LinkForm
...
#         Comment.objects.filter(type_id=type_id, resource_id=note_id).delete()
...
#         try:
#             note = Note.objects.get(id=note_id)
#             comments = Comment.objects.filter(type_id=type_id, resource_id=note_id).order_by('-date')
#             user_id = None
#             if request.user.is_authenticated():
#                 is_auth_user = True
...
#
#         Comment(user=request.user, comment=cleaned_comment, resource_id=resource_id, type_id=type_id, date=date).save()
